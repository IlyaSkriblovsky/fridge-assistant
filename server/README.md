...
Тесты не требуют `.env`, сети, прибора или аккаунтов Google. Gemini и Keep
подменены; заметки Keep и WAV создаются в памяти. Проверяются бизнес-логика,
цикл ассистента, HTTP-контракт и обработка потоковой загрузки.
В CI тесты обязательны перед сборкой и публикацией Docker-образа.
